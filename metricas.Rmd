 ...
 * Importar e plotar dados raster em R e sobrepor locais de amostragem.  
 
 * Calcular métricas de paisagem.  
 
 * Extrair dados de paisagem em locais de amostragem e dentro de um buffer ao redor deles.  
 

# Codigo e R
Objetivo não é de apresentar detalhes sobre os métodos ou os funções no [R](https://cran.r-project.org/). Existem diversos exemplos disponíveis, por exemplo:  

 * livros online:    
 
    * [Ciência de Dados com R](https://cdr.ibpad.com.br/cdr-intro.pdf)     

    * [Análises Ecológicas no R](https://analises-ecologicas.com/index.html)

 * Com Google "r cran introdução tutorial"..... onde tem mais de 50 mil resultados com exemplos, imagens e videos ......  
...
[landscapemetrics](https://r-spatialecology.github.io/landscapemetrics/) é um pacote R para calcular métricas de paisagem em paisagems categóricos (onde tem uma classificação de cobertura de terra/habitat), em um fluxo de trabalho organizado. O pacote pode ser usado como um substituto do FRAGSTATS ([McGarigal et al. 1995](https://doi.org/10.2737/PNW-GTR-351),  https://doi.org/10.2737/PNW-GTR-351), pois oferece um fluxo de trabalho reproduzível para análise de paisagem em um único ambiente (Professor McGarigal se aposentou, então o software original não é mais suportado).
 ...
 * Daí você deve incluir o nível da análise “p” para patch (ou seja, para a mancha ou fragmento), “c” para classe e “l” para landscape ou seja, para métricas para a paisagem como um todo.   

 * E daí existem inúmeras métricas, como por exemplo a `r kableExtra::text_spec("cpland", background = "#dedede")` que é o percentual de área central - “core area” na paisagem, como vimos na aula teórica. Assim sendo, a função `r kableExtra::text_spec("lsm_c_cpland", background = "#dedede")` vai calacular a métrica porcentagem da área central em cada classe.  

Digite o código abaixo e veja o resultado. Leia com atenção e preste particular atenção na organização da página de ajuda.
...
Aqui, precisamos o arquivo com os dados de MapBiomas "amostra_mapbiomas_2020.tif", que voces baixaram no tutorial anterior (você pode baixar novamente seguindo tutorial Escala https://rpubs.com/darren75/escala).
...
Primeiramente carregamos os dados e em seguida converter as coordenadas para o mesmo sistema de referência que o arquivo raster (com a função st_transform). 
...
Note que no codigo a seguir, que, adicionamos um geom com um “+”. No ggplot2, nós criamos gráficos em camadas, e adicionamos camada a camada com um “+”. Assim, é posivel ajustar qualquer elemento do grafico.

Aqui vamos fazer um gráfico com valores de extensão no eixo x e proporção da floresta central no eixo y.
Assim sendo, com o codigo a segir, vamos informar (i) os dados, selecionando classe de floresta atraves de um filtro e acresentando uma coluna nova ("ext_m") com a extensão em metros, (ii) as colunas para os eixos x e y, (iii) tipo de grafico (grafico de pontos - `r kableExtra::text_spec("geom_point()", background = "#dedede")` e grafico de linha - `r kableExtra::text_spec("geom_line()", background = "#dedede")`), (iv) nomes para os eixos. No exemplo, usamos `r kableExtra::text_spec("%>%", bold = TRUE)`, que estabelece a ligação entre os passos do processo, ligando os dados (amostras_metrica) e o grafico `r kableExtra::text_spec("ggplot", background = "#dedede")`.
```{r, eval=FALSE}
# arrumar os dados
amostras_metrica %>% 
  filter(class==1) %>% 
...
`r kableExtra::text_spec("Em vez de extensão, você preciso incluir o tamanho (área) correspondente a cada raio ([área de um circulo](https://www.ensinobasico.com/blogue/1482-area-e-perimetro-de-um-circulo)). Incluir uma cópia do código ajustado para produzir uma figura com tamanho (área em quilômetros quadrados) no eixo x e a percentual de área central de floresta no eixo y.", color = "blue", bold = TRUE)`
