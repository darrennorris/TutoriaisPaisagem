...
O comando principal função a ser utilizado se chama ggplot (sem o "2").
...
Depois da função ggplot, nós especificamos qual tipo de grafico com um geom. Por exemplo, geom_point para plotar pontos, geom_boxplot para um boxplot, etc. Para a lista completa de geoms e todas as outras opções do pacote, visite a página do projeto ggplot2, 
...
Assim sendo, com o codigo a segir, vamos informar (i) os dados (selecionando classe de floresta atraves de um filtro), (ii) as colunas para os eixos x e y, (iii) tipo de grafico (grafico de pontos), (iv) nomes para os eixos.
```{r, eval=FALSE}
amostras_metrica %>% 
  filter(class==1) %>%
